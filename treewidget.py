...
# TreeWidget
# Copyright (C) 2001-2008 Jean-Baptiste LAMY -- jiba@tuxfamily
#
...
def create_default_iconsdir():
  global iconsdir
  import editobj2
  iconsdir = IconsDir(editobj2._ICON_DIR)
...
"""
  ...
  def update(self):
    ...
    image = self.geticon()
    if not self._icon is image:
      self._icon = image
      self.tree.itemconfigure(self.image_id, image = image)
  def updatetree(self):
    self.update()
  ...
  def select(self, event = None):
    if not self.selected:
      ...
      try: self.label.configure(bg = "#CCCCFF")#bg = "#834cb4")
      except: pass
      self.redraw()
  def deselect(self, event = None):
    if self.selected:
      ...
      try: self.label.configure(bg = self.tree["bg"])
      except: pass
      self.redraw()
  ...
  def _drawtext(self, x, y):
    ...
    try: label = self.label
      ...
      self.label = Label(self.tree, text = unicode(self), fg = "black", bg = self.tree["bg"], bd = 0, padx = 2, pady = 2)
...
class ScrollPane(Canvas):
  ...
  def _resized(self, event = None):
    ...
    if (width  > 10) and not self.scrollX: self.itemconfigure(self.id, width  = width)
    if (height > 10) and not self.scrollY: self.itemconfigure(self.id, height = height)
...
As the drawing is by default asynchronous, you can call draw() a lot of time without perf loss."""
  ...
  def yview_scroll(self, *args):
    if args[1][:1] == "u":
      self.yview_moveto(float(self.y + int(args[0])) / self.sizetree)
    else:
      ...
      nbincrements = int(args[0]) * (height / self.nodeheight)
